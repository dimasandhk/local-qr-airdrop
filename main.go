...
<head>
	...
	<style>
		:root { --bg-color: #ffffff; --text-color: #333333; --card-bg: #ffffff; --card-border: #dddddd; --input-border: #cccccc; --btn-bg: #007bff; --btn-hover: #0056b3; }
		[data-theme="dark"] { --bg-color: #1a1a1a; --text-color: #f0f0f0; --card-bg: #2d2d2d; --card-border: #444444; --input-border: #555555; --btn-bg: #0d6efd; --btn-hover: #0b5ed7; }
		...
		input[type=file] { margin: 20px 0; padding: 10px; border: 1px solid var(--input-border); border-radius: 5px; width: 100%%; box-sizing: border-box; background-color: var(--bg-color); color: var(--text-color); }
	...
	<script>
		...
		window.addEventListener('DOMContentLoaded', () => {
			document.getElementById('theme-icon').innerText = getTheme() === 'dark' ? '☀️' : '🌙';
		});
	</script>
</head>
<body>
...
<head>
	...
	<style>
		...
		[data-theme="dark"] { --bg-color: #1a1a1a; --text-color: #f0f0f0; --btn-bg: #198754; --btn-hover: #157347; }
		body { font-family: sans-serif; padding: 20px; text-align: center; max-width: 600px; margin: auto; background-color: var(--bg-color); color: var(--text-color); transition: background-color 0.3s, color 0.3s; }
