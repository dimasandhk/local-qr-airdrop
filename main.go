...
<head>
	...
	<script>
		...
		document.addEventListener("DOMContentLoaded", () => setTheme(document.documentElement.getAttribute('data-theme')));
	</script>
	<style>
		:root {
			--bg-color: #ffffff; --text-color: #333333; --card-bg: #f9f9f9; --card-border: #ddd;
			--input-border: #ccc; --input-bg: #ffffff; --btn-bg: #007bff; --btn-hover: #0056b3; --btn-text: #ffffff;
		}
		[data-theme="dark"] {
			--bg-color: #121212; --text-color: #e0e0e0; --card-bg: #1e1e1e; --card-border: #333;
			--input-border: #555; --input-bg: #2a2a2a; --btn-bg: #bb86fc; --btn-hover: #9b59b6; --btn-text: #000000;
		}
		...
		.btn { background: var(--btn-bg); color: var(--btn-text); border: none; padding: 12px 24px; border-radius: 5px; font-size: 16px; margin-top: 20px; width: 100%%; cursor: pointer; transition: background 0.3s; }
		.btn:hover { background: var(--btn-hover); }
		input[type=file] { margin: 20px 0; padding: 10px; border: 1px solid var(--input-border); background-color: var(--input-bg); color: var(--text-color); border-radius: 5px; width: 100%%; box-sizing: border-box; }
		.card { border: 1px solid var(--card-border); background-color: var(--card-bg); padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); transition: background-color 0.3s, border-color 0.3s; }
		.theme-toggle-container { text-align: right; margin-bottom: 10px; }
		#theme-toggle { background: transparent; border: none; font-size: 24px; cursor: pointer; color: var(--text-color); padding: 0; }
	</style>
</head>
<body>
...
<head>
	...
	<style>
		:root {
			...
			--btn-bg: #28a745; --btn-hover: #218838; --btn-text: #ffffff;
		}
		[data-theme="dark"] {
			--bg-color: #121212; --text-color: #e0e0e0;
			--btn-bg: #bb86fc; --btn-hover: #9b59b6; --btn-text: #000000;
		}
		body { font-family: sans-serif; padding: 20px; text-align: center; max-width: 600px; margin: auto; background-color: var(--bg-color); color: var(--text-color); transition: background-color 0.3s, color 0.3s; }
		.btn { background: var(--btn-bg); color: var(--btn-text); border: none; padding: 12px 24px; border-radius: 5px; font-size: 16px; margin-top: 20px; text-decoration: none; display: inline-block; transition: background 0.3s; }
		.btn:hover { background: var(--btn-hover); }
		.theme-toggle-container { text-align: right; margin-bottom: 10px; }
		#theme-toggle { background: transparent; border: none; font-size: 24px; cursor: pointer; color: var(--text-color); padding: 0; }
	</style>
