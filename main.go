...
<head>
	...
	<style>
		...
		input[type=file] { margin: 20px 0; padding: 10px; border: 1px solid var(--border-color); border-radius: 5px; width: 100%%; box-sizing: border-box; background-color: var(--bg-color); color: var(--text-color); }
		...
		#theme-toggle { position: absolute; top: 10px; right: 10px; cursor: pointer; background: none; border: none; font-size: 24px; }
	</style>
	<script>
		(function() {
...
<body>
	...
	<script>
		...
		function updateIcon() {
			themeToggle.textContent = docEl.getAttribute('data-theme') === 'dark' ? '☀️' : '🌙';
		}
		updateIcon();
...
<body>
	...
	<a href="/" class="btn btn-success">Upload Another File</a>
	<script>
		const themeToggle = document.getElementById('theme-toggle');
		const docEl = document.documentElement;
		function updateIcon() {
