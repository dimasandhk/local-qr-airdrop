...
<head>
	...
	<style>
		:root { --bg: #fff; --text: #000; --card-bg: #fff; --border: #ddd; }
		:root.dark-mode { --bg: #121212; --text: #e0e0e0; --card-bg: #1e1e1e; --border: #333; }
		body { font-family: sans-serif; padding: 20px; text-align: center; max-width: 600px; margin: auto; background-color: var(--bg); color: var(--text); transition: background-color 0.3s, color 0.3s; }
		...
		input[type=file] { margin: 20px 0; padding: 10px; border: 1px solid var(--border); border-radius: 5px; width: 100%%; box-sizing: border-box; background-color: var(--bg); color: var(--text); }
		.card { border: 1px solid var(--border); padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); background-color: var(--card-bg); }
		#theme-icon { position: absolute; top: 20px; right: 20px; background: none; border: none; font-size: 24px; cursor: pointer; }
	</style>
	<script>
		function initTheme() {
...
<body>
	<button id="theme-icon" onclick="toggleTheme()">🌓</button>
	<div class="card">
...
<head>
	...
	<style>
		...
		:root.dark-mode { --bg: #121212; --text: #e0e0e0; }
		body { font-family: sans-serif; padding: 20px; text-align: center; max-width: 600px; margin: auto; background-color: var(--bg); color: var(--text); transition: background-color 0.3s, color 0.3s; }
